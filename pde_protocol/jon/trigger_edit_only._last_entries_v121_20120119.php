...
if($newvals['protocol_number'] != $oldvals['protocol_number']) $newvals['protocol_number'] = $oldvals['protocol_number']; 
if($newvals['protocol_year'] != $oldvals['protocol_year']) $newvals['protocol_year'] = $oldvals['protocol_year']; 

$currentYear=getCurrentYear();
$entries_allowed_to_edit=50;



//print_r($oldvals);//debug
$query2 = "SELECT MAX(protocol_number) as maxnum FROM protocol_entries WHERE protocol_year=".$currentYear; 
//
if($data=$this->MyQuery($query2)){ 
   ...
   //$this->fdd['excavation']['input'] = 'R';
		...
		if($this_protocol_year==$currentYear){
