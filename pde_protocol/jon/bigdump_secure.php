<?php
/*
// BigDump ver. 0.34b-trimod004 20120502c -Removed replacement of GET URL to allows subfolder import/delete -MIGHT BE SUCIRUTY HOLE !!!!

Changes:
// BigDump ver. 0.34b-trimod003 20120502 -Removed replacement of GET URL to allows subfolder import/delete -MIGHT BE SUCIRUTY HOLE !!!!
// BigDump ver. 0.34b-trimod002 20120502 -Now getting dirlist of another folder
// BigDump ver. 0.34b-trimod001 20120430 -implemented phpSecurePages
// BigDump ver. 0.34b from 2011-09-04 ORIGINAL  version
...
$use_phpSecurePages=true;// added by tri
$upload_dir_modify="/".$upload_dir_modify_no_slash;//added by tri 20120502 - use this to change default dir
$upload_dir_modify_no_slash="backup_sql_/";//added by tri 20120502 - use this to change default dir
$upload_dir_modify="/".$upload_dir_modify_no_slash;//added by tri 20120502 - use this to change default dir
$debug=true;

//*********NOTE requires phpSecurePages *************
if($use_phpSecurePages){
$minUserLevel=7; // compared to $userLevel 
...
$db_server   = 'localhost';
$db_name     = 'pde_protocol_production';
$db_username = 'john';
$db_password = 'john'; 
...
{
  ///$val = preg_replace("/[^_A-Za-z0-9-\.&= ;\$]/i",'', $val);  // Commented 20120502 FOR IMPORT & DELETE TO WORK - SECURITY ISSUE
  ///$_REQUEST[$key] = $val; // Commented 20120502 FOR IMPORT & DELETE TO WORK - SECURITY ISSUE
} // Commented 20120502 FOR IMPORT & DELETE TO WORK - SECURITY ISSUE
...
if (isset($_SERVER["CGIA"]))
  ...
  $upload_dir_original=$upload_dir;// added by jon 20120502
  $upload_dir=$upload_dir.$upload_dir_modify;

  if($debug)echo "<h1>upload_dir=$upload_dir</h1>";
  echo "<h1>Database name : $db_name</h1>";
// Handle file upload
...
{ 
//echo"<h2>Remote Delete is DISABLED</h2>"; // disabled DELETE did not work for different folders
//if($debug) echo"<hr>DELETE $upload_dir_modify_no_slash".basename($_REQUEST["delete"]);


if (preg_match("/(\.(sql|gz|csv))$/i",$_REQUEST["delete"]) && @unlink($upload_dir_modify_no_slash.basename($_REQUEST["delete"])))
...
//if($debug)echo "<h1>upload_dir2=$upload_dir</h1>";//debug
...
{ if ($dirhandle = opendir($upload_dir)) 
  { 
    ...
    { 
      ...
      { 
		//if($debug) echo"<hr>$upload_dir_modify_no_slash.$dirfile";
        ...
        { if (!$dirhead)
          ...
          echo ("<tr><td>$dirfile</td><td class=\"right\">".filesize($upload_dir_modify_no_slash.$dirfile)."</td><td>".date ("Y-m-d H:i:s", filemtime($upload_dir_modify_no_slash.$dirfile))."</td>");
          ...
          if ((preg_match("/\.gz$/i",$dirfile) && function_exists("gzopen")) || preg_match("/\.sql$/i",$dirfile) || preg_match("/\.csv$/i",$dirfile))
            echo ("<td><a href=\"".$_SERVER["PHP_SELF"]."?start=1&amp;fn=".urlencode($upload_dir_modify_no_slash.$dirfile)."&amp;foffset=0&amp;totalqueries=0&amp;delimiter=".urlencode($delimiter)."\">Start Import</a> into $db_name at $db_server</td>\n <td><a href=\"".$_SERVER["PHP_SELF"]."?delete=".urlencode($upload_dir_modify_no_slash.$dirfile)."\">Delete file</a></td></tr>\n");
          else
            echo ("<td>&nbsp;</td>\n <td>&nbsp;</td></tr>\n");
        }
      }
...
// Recognize GZip filename
  ...
  if (preg_match("/\.gz$/i",$curfilename)) 
    ...
    $gzipmode=false;
if($debug) echo"<h1>curfilename =$curfilename;</h1>";	
//$curfilename=$upload_dir_original.$curfilename;
if($debug) echo"<h1>curfilename =$curfilename;</h1>";
...
//Next line is for logout from phpSecurePages
if($use_phpSecurePages) echo "<A HREF='logout_jon.php' target='_main' >ΕΞΟΔΟΣ</a>";
?>
